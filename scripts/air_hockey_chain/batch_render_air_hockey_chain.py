...
# the two knobs that change how the relay runs without changing what it is: the
# push and the mallets' restitution. Both ranges were checked at their corners
# against simulate_air_hockey_chain.py -- at all four the relay still completes,
# every striker is still left under an eighth of its speed, and none of them
# rebounds. What moves is the timing of the two handoffs and how far up the near
# rail the last mallet finishes.
PUSH_SPEED_RANGE = (0.72, 0.90)
MALLET_RESTITUTION_RANGE = (0.92, 0.97)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch render randomized air-hockey mallet relay videos with Blender."
    )
    parser.add_argument(
        "--out-root",
        type=Path,
        default=WORKSPACE_DIR / "renders" / "batch_air_hockey_chain",
    )
    ...
    parser.add_argument("--preview-frame", type=int, default=31)
...
def sample_output_name(mode: str) -> str:
    return "preview.png" if mode == "preview" else "air_hockey_chain.mp4"


def sample_params(seed: int) -> dict[str, float]:
    rng = random.Random(seed)
    return {
        "push_speed": round(rng.uniform(*PUSH_SPEED_RANGE), 3),
        "mallet_restitution": round(rng.uniform(*MALLET_RESTITUTION_RANGE), 3),
    }


def render_command(
    args: argparse.Namespace, out_dir: Path, params: dict[str, float]
) -> list[str]:
    return [
        ...
        "--push-speed", str(params["push_speed"]),
        "--mallet-restitution", str(params["mallet_restitution"]),
    ]
...
def resolve_blender_path(configured: Path | None) -> Path:
    ...
    raise FileNotFoundError(
        f"Blender executable not found. Pass --blender, set BLENDER_BIN, or place "
