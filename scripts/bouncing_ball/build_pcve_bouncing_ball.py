...
CASES = (
    RenderCase(
        case_id="bounce_baseline",
        description="Baseline free-fall bounce with default physics.",
        seed=4001,
        overrides={},
    ),
    RenderCase(
        case_id="bounce_high_restitution",
        description="Higher ball restitution leads to more and longer bounces.",
        seed=4002,
        overrides={
            "physics": {
    ...
    RenderCase(
        case_id="bounce_heavy_ball",
        description="Heavier ball carries more momentum and compresses bounces differently.",
        seed=4003,
        overrides={
            "physics": {
                "ball_mass": 1.8,
    ...
    RenderCase(
        case_id="bounce_lateral_velocity",
        description="Initial lateral velocity makes the ball bounce across the floor.",
        seed=4004,
        overrides={
            "physics": {
                "ball_initial_velocity": [1.0, 0.0, 0.0],
            },
        },
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a PCVE synthetic bouncing ball benchmark suite."
    )
    parser.add_argument(
        "--out-root",
        type=Path,
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "description": "Single rigid bouncing ball cases with PyBullet-driven trajectories for PCVE evaluation.",
