...
CASES = (
    RenderCase(
        ...
        description="Baseline bowling strike with default physics.",
        seed=5001,
        overrides={},
    ),
    RenderCase(
        case_id="bowling_heavy_ball",
        description="Heavier bowling ball hits pins with more momentum.",
        seed=5002,
        overrides={
            "physics": {
                "ball_mass": 5.0,
    ...
    RenderCase(
        case_id="bowling_off_center",
        description="Ball hits the pins off-center with lateral velocity.",
        seed=5003,
        overrides={
            "physics": {
                "ball_initial_velocity": [-8.0, 1.5, 0.0],
            },
        },
    ),
    RenderCase(
        case_id="bowling_high_speed",
        description="Faster ball speed creates a more energetic collision.",
        seed=5004,
        overrides={
            "physics": {
                "ball_initial_velocity": [-11.0, 0.0, 0.0],
            },
        },
    ),
    RenderCase(
        case_id="bowling_high_restitution",
        description="Higher pin restitution makes pins bounce farther after impact.",
        seed=5005,
        overrides={
            "physics": {
                "pin_restitution": 0.6,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a PCVE synthetic bowling benchmark suite."
    )
    parser.add_argument(
        "--out-root",
        type=Path,
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "description": "Bowling ball hitting pins cases with PyBullet-driven trajectories for PCVE evaluation.",
