...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch render randomized car gap-jump videos with Blender."
    )
    ...
    parser.add_argument("--preview-frame", type=int, default=52)
...
def sample_params(seed: int) -> dict[str, float]:
    # Each sample jitters the launch speed and the gap width within ranges that
    ...
    return {
        "launch_speed": round(rng.uniform(9.5, 13.5), 2),
        "gap_width": round(rng.uniform(7.0, 11.0), 2),
    }
