...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch render randomized indoor toy-car gap-jump videos with Blender."
    )
    parser.add_argument("--out-root", type=Path, default=WORKSPACE_DIR / "renders" / "batch_car_gap_jump")
    parser.add_argument("--count", type=int, default=4)
    parser.add_argument("--start-index", type=int, default=0)
    parser.add_argument("--seed-base", type=int, default=31000)
...
def sample_output_name(mode: str) -> str:
    return "preview.png" if mode == "preview" else "car_gap_jump.mp4"


def sample_params(seed: int) -> dict[str, float]:
    # Each sample jitters the push speed and the gap width within ranges that
    # keep the jump interesting -- some clear, some fall short -- rather than
    # always succeeding. The speed band straddles the clear/fail threshold for
    # the sampled gaps (about 1.55 m/s at a 0.28 m gap). The top of the speed
    # band is capped so the car always comes to rest well short of the vase
    # standing further along the landing table.
    rng = random.Random(seed)
    return {
        "launch_speed": round(rng.uniform(1.0, 2.2), 2),
        "gap_width": round(rng.uniform(0.22, 0.42), 2),
    }
...
def render_command(args: argparse.Namespace, *, out_dir: Path, seed: int, params: dict[str, float]) -> list[str]:
    return [
        ...
        "--gap-width", str(params["gap_width"]),
    ]
