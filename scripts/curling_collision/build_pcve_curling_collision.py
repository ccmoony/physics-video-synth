...
CASES = (
    RenderCase(
        ...
        description="Equal-mass head-on collision: both stones come to rest at the point of impact.",
        seed=9001,
        overrides={},
    ),
    RenderCase(
        case_id="curling_collision_unequal_mass",
        description="The incoming stone is much heavier than the target, so momentum is not "
        "canceled: the light stone is knocked backward while the heavy stone keeps "
        "moving forward, instead of both stopping.",
        seed=9002,
        overrides={
            "physics": {
                "stone_2_mass": 34.0,
    ...
    RenderCase(
        case_id="curling_collision_bouncy",
        description="Higher restitution turns the collision elastic: the stones bounce apart "
        "and keep moving instead of both stopping dead.",
        seed=9003,
        overrides={
            "physics": {
                "stone_restitution": 0.6,
            },
        },
    ),
    RenderCase(
        case_id="curling_collision_high_ice_friction",
        description="Much higher ice friction bleeds off speed before the stones meet, so they "
        "never actually collide -- both stop short, apart from each other.",
        seed=9004,
        overrides={
            "physics": {
                "ice_friction": 0.15,
    ...
    RenderCase(
        case_id="curling_collision_fast_launch",
        description="A much higher launch speed makes for a harder, more violent impact.",
        seed=9005,
        overrides={
            "physics": {
                "launch_speed": 2.2,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a PCVE synthetic curling stone collision benchmark suite."
    )
    parser.add_argument(
        "--out-root",
        type=Path,
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "description": "Head-on curling stone collision cases with PyBullet-driven trajectories "
        "for PCVE evaluation, covering momentum transfer (mass asymmetry), elastic vs. "
        "inelastic impact (restitution), and whether the stones ever meet at all (ice friction).",
