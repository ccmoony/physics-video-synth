...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch render randomized picnic apple/ball impact videos with Blender."
    )
    parser.add_argument("--out-root", type=Path, default=WORKSPACE_DIR / "renders" / "batch_picnic_apple_ball")
...
def sample_output_name(mode: str) -> str:
    return "preview.png" if mode == "preview" else "picnic_apple_ball.mp4"


def sample_params(seed: int) -> dict[str, float]:
    # Each sample jitters the hit's lever arm, the drop height, and the grass
    # friction within ranges that keep a clearly visible roll (the whole point
    # of the scene) but vary its length and timing. The offset floor of
    # 0.06 m stays safely above the "dead-center, no roll" threshold, and the
    # friction ceiling keeps the ball from rolling out of frame.
    rng = random.Random(seed)
    return {
        "apple_offset_x": round(rng.uniform(0.08, 0.125), 4),
        "drop_height": round(rng.uniform(1.1, 1.8), 3),
        "grass_friction": round(rng.uniform(0.18, 0.32), 3),
    }


def render_command(args: argparse.Namespace, *, out_dir: Path, seed: int, params: dict[str, float]) -> list[str]:
    return [
        str(args.blender),
        "-b", "--python", str(RENDER_SCRIPT), "--",
        "--mode", str(args.mode),
        "--out-dir", str(out_dir),
        ...
        "--seed", str(int(seed)),
        "--apple-offset-x", str(params["apple_offset_x"]),
        "--drop-height", str(params["drop_height"]),
        "--grass-friction", str(params["grass_friction"]),
    ]
...
def main() -> None:
    ...
    for offset in range(int(args.count)):
        ...
        expected_output = sample_dir / sample_output_name(args.mode)
        command = render_command(args, out_dir=sample_dir, seed=seed, params=params)
        sample_record = {
