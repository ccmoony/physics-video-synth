...
def _ensure_physics_env_on_path() -> None:
    """Prepend the physics conda env to PATH so Blender can find PyBullet."""
    candidates = [
        WORKSPACE_DIR.parent / "miniconda3" / "envs" / "physics" / "bin",
        WORKSPACE_DIR.parent / "miniconda" / "envs" / "physics" / "bin",
        Path.home() / "miniconda3" / "envs" / "physics" / "bin",
        Path.home() / "miniconda" / "envs" / "physics" / "bin",
    ]
...
CASES = (
    RenderCase(
        case_id="pool_baseline",
        description="Baseline head-on pool collision with default physics.",
        seed=3001,
        overrides={},
    ),
    RenderCase(
        case_id="pool_high_restitution",
        description="Higher ball restitution produces a livelier collision.",
        seed=3002,
        overrides={
            "physics": {
    ...
    RenderCase(
        case_id="pool_heavy_cue",
        description="Heavier cue ball carries more momentum into the target ball.",
        seed=3003,
        overrides={
            "physics": {
                "ball_mass": 0.25,
    ...
    RenderCase(
        case_id="pool_off_center",
        description="Off-center impact causes the balls to separate at an angle.",
        seed=3004,
        overrides={
            "physics": {
                "cue_initial_location": [0.06, -0.6, 0.0],
                "cue_initial_velocity": [0.0, 1.0, 0.0],
            },
        },
    ),
    RenderCase(
        case_id="pool_fast_break",
        description="Faster cue ball increases post-collision speeds.",
        seed=3005,
        overrides={
            "physics": {
                "cue_initial_velocity": [0.0, 2.0, 0.0],
            },
        },
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a PCVE synthetic pool collision benchmark suite."
    )
    parser.add_argument(
        "--out-root",
        type=Path,
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "description": "Pool collision cases with PyBullet-driven trajectories for PCVE evaluation.",
