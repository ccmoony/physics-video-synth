...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch render tennis-ball-off-a-coffee-table videos with Blender.",
    )
    parser.add_argument("--out-root", type=Path,
                        default=WORKSPACE_DIR / "renders" / "batch_table_drop_collision")
    ...
    parser.add_argument(
        "--preview-frame", type=int, default=21,
        help="Frame 21 is one frame before contact, with the falling ball clear "
        "of the table and the target still where it started, so a still says "
        "whether the shot lines up at all.",
    )
...
def sample_output_name(mode: str) -> str:
    ...
    return "table_drop_collision.mp4"
